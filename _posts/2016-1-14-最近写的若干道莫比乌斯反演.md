...
$$d(nm)=\sum_{i=1}^{n}\sum_{j=1}^{m}[gcd(i,j)==1]$$
...
那么$$p$$对左式贡献是$$k1+k2+1$$,右式贡献也是$$k1+k2+1$$.证明完毕

故可得
$$\sum_{i=1}^{n}\sum_{j=1}^{m}d(ij)=\sum_{i=1}^{n}\sum_{j=1}^{m}\lfloor \frac{n}{i} \rfloor \lfloor \frac{m}{i} \rfloor \times[gcd(i,j)==1]\\
=\sum_{i=1}^{n}\sum_{j=1}^{m}\lfloor \frac{n}{i} \rfloor \lfloor \frac{m}{i} \rfloor \sum_{d|gcd(i,j)}\mu(d)\\
