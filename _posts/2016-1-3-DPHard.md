...
### 2015.1.5 SRM 603 1000pts

题意:给你两个数组$$A[i],B[i]$$,任意排列$$A,B$$使得$$C[i]=A[i]+B[i]$$的众数出现次数最多.$$A[i],B[i] \leq 10^5,n \leq 10^5 $$

**sol:**

先考虑一个暴力...对于数$$x$$,显然答案为$$\sum_i min(sum[i],sum[x-i])$$

又有$$\sum_i min(sum[i],sum[x-i])=\sum_k \sum_i [sum[i]>=k]\times [sum[x-i]>=k]$$

于是对$$k$$进行分块..$$k \leq limit$$时FFT,$$k \geq limit$$时暴力

复杂度为$$O(\frac{n}{L}^2+Lnlogn)$$
