...
D:令$$f[i][j]$$表示做到第$$i$$种,两个人相差$$j$$的方案数..直接暴力转移不太行..然后大概找找规律dp一下就行...

E:一道神奇的魔方题..

F:令$$f[i]$$表示$$i$$的子树的答案,对于每一条链用lca去统计答案.记$$sum[i]=\sum_{fa[s]=i}f[s]$$,那么每次链转移就相当于求一条链上的$$sum[i]-d[i]$$,这个只要dfs序维护一发就好

G:裸最小割..唯一有问题的是题意．

H:

I:大概就是加点删点维护虚树边权.设加入点是$$x$$,它在dfs序上左边最近点是$$x$$,右边最近点是$$y$$,那么新加的贡献就是$$dis[u]-dis[lca(u,x)]-dis[lcs(u,y)]+dis[lca(x,y)]$$,因为就相当于把$$u$$并到$$x->y$$的链上,**这个万一还是要记一记的**
